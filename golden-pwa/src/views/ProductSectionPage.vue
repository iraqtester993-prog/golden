<template>
  <div class="page"><TopBar :title="sectionTitle" :showBack="true"/><main class="content"><div class="section-head"><span>{{ sectionTitle }}</span><small>{{ products.length }} منتجات</small></div><div class="products-grid"><article v-for="product in products" :key="product.name" @click="router.push('/product?name='+encodeURIComponent(product.name))"><img :src="product.img"/><div><h2>{{ product.name }}</h2><strong><small>السعر النقدي</small>{{ product.price }} د.ع</strong><div class="installment"><span class="material-symbols-outlined">account_balance_wallet</span> يدعم التقسيط</div><div class="cart-control" @click.stop><button @click="add(product)"><span class="material-symbols-outlined">add_shopping_cart</span>إضافة للسلة</button></div></div></article></div></main><BottomNav/></div>
</template>
